...
# another way
min(penguins$body_mass_g, na.rm = T)
...
penguin_long_flipper <- subset(penguins, flipper_length_mm > 210)
# look at the data frame. what do you notice about these animals (esp their species)?
View(penguin_long_flipper)
# all but one penguin are Gentoo species
```
...
print(penguin_bill_length_summary)
```
...
print(penguin_bill_length_summary_2)
```
...
penguin_bill_depth_summary <- ddply(penguins,
                                    ~species,
                                    summarize,
                                    mean_bill_depth = mean(bill_depth_mm, na.rm = T),
                                    se_bill_depth = sd(bill_depth_mm, na.rm = T)/sqrt(sum(!is.na(bill_depth_mm))-1))
print(penguin_bill_depth_summary)
```
...
penguin_bill_length_summary_3 <-
  ddply(penguins,
        ...
        mean_bill_Length = mean(bill_length_mm, na.rm = T),
        sd_bill_Length = sd(bill_length_mm, na.rm = T))
print(penguin_bill_length_summary_3)
```
