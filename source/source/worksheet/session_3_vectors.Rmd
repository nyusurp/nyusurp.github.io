...
- [Get the source of this Rmd file](https://raw.githubusercontent.com/nyusurp/nyusurp.github.io/master/source/worksheet/session_3_vectors.Rmd)
- [Go to solution sheet](source/solution/session_3_vectors.html)
- [The slide deck for the session](slide/session_3_vectors.html)
...
Previously, we have seen that failed conversion of types could create `NA`s in your data. If `NA`s are introduced because of your analysis pipeline, there's a good chance that you would want to avoid it or correct it manually.

When working with large datasets, you might not be able to notice `NA`s, but there are functions that would inspect the data for us.

```{r}
# is.na() takes a vector and returns a logical vector (TRUE/FALSE).
# The corresponding items in the original vector to TRUE items in the returned vector are NAs

## 8R is a typo when I keyed in my data
expression <- c(1, 2, 5, "8R")

## If I convert the vector to numeric, NAs will be created.
expression_num <- as.numeric(expression)
print(expression_num)
```

```{r}
# is.na() returns a logical vector showing the items that are NAs
is.na(expression_num)
```
