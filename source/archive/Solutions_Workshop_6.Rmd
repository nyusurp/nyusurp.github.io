...
Try the following:
```{r}
# Calculate the mean bill length for all penguins in the dataset
mean(penguin_df$bill_length_mm, na.rm = T)
# Calculate the standard deviation of bill length for all penguins in the dataset
sd(penguin_df$bill_length_mm, na.rm = T)
```
...
penguin_df$body_mass_kg <- penguin_df$body_mass_g/1000
```
...
# colMeans(parks_df) - this doesn't work because some of the columns don't have numeric data
# do you run into problems? why? how could we get what we want using this function?
colMeans(parks_df[, c('squirrels', 'rats', 'pigeons')])
```
